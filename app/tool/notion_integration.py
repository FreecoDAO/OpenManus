    ...
    async def _query_database(
        ...
        for page in response.get('results', []):
            ...
            for prop_name, prop_value in page.get('properties', {}).items():
                ...
                if prop_type == 'title':
                    title_array = prop_value.get('title', [])
                    properties[prop_name] = ''.join([t.get('plain_text', '') for t in title_array])
                elif prop_type == 'rich_text':
                    text_array = prop_value.get('rich_text', [])
                    properties[prop_name] = ''.join([t.get('plain_text', '') for t in text_array])
        ...
        try:
            ...
            if action == "create_page":
                ...
                if 'page_id' not in kwargs or 'content' not in kwargs:
                    return self.error_response("Missing required parameters: page_id, content")
                ...
                if 'database_id' not in kwargs:
                    return self.error_response("Missing required parameter: database_id")
