...
async def create_plan_with_tot(self, request: str, num_alternatives: int = 3) -> None:
    ...
    """
    logger.info(f"Creating plan with Tree-of-Thoughts (alternatives={num_alternatives})")
    
    # Initialize ToT if not already done
    if self.tree_of_thoughts is None:
        self.tree_of_thoughts = TreeOfThoughts(
            max_depth=2,  # Root + alternatives
            max_branches=num_alternatives
        )
    ...
    try:
        ...
        root = self.tree_of_thoughts.add_thought(
            ...
            metadata={"type": "root", "task": request}
        )
        ...
        await self.tree_of_thoughts.evaluate_all_thoughts(
            llm=planning_llm,
            evaluation_prompt=TOT_EVALUATION_PROMPT
        )
        ...
        if len(best_path) < 2:
            logger.warning("ToT did not produce valid alternatives, falling back to standard planning")
