...
class GracefulDegradation:
    ...
    def register_provider(
        ...
        logger.info(f"Registered provider: {service}/{provider_name} (priority={priority})")
    ...
    def switch_provider(self, service: str, reason: str = "") -> Optional[ServiceProvider]:
        """
        ...
        for provider in providers:
            if provider.is_available and provider.health_score > 0.3:
                logger.info(f"Switched to provider: {service}/{provider.name} - {reason}")
                return provider
