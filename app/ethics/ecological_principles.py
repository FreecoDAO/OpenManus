...
class EcologicalSystem:
    ...
    def optimize_energy(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "max_tokens" in task and task.get("priority", "normal") != "high":
            ...
            if optimized["max_tokens"] < original_tokens:
                logger.info(f"Reduced max_tokens: {original_tokens} -> {optimized['max_tokens']}")
    ...
    def track_carbon(self, operation: str, **kwargs) -> CO2Metric:
        ...
        if "llm_tokens" in kwargs:
            token_co2 = (kwargs["llm_tokens"] / 1000) * self.co2_factors["llm_1k_tokens"]
            co2_grams += token_co2
    ...
    def get_sustainability_metrics(self) -> SustainabilityMetrics:
        ...
        reuse_rate = self.resources_reused / total_resources if total_resources > 0 else 0.0
...
class OperationTracker:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        logger.debug(f"Operation {self.operation_name} completed in {duration_ms:.1f}ms")
